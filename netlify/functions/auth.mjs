...
export const handler = async (event) => {
  ...
  if (action === 'login') {
    ...
    if (email.toLowerCase() !== ALLOWED_EMAIL) return { statusCode: 403, headers, body: JSON.stringify({ error: 'Access is currently restricted' }) };
    const result = await sb('GET', `/pt_users?email=eq.${encodeURIComponent(email.toLowerCase())}&select=*`);
    if (!result.ok || !result.data?.length) return { statusCode: 401, headers, body: JSON.stringify({ error: 'No account found with that email.' }) };
    const user = result.data[0];
    if (user.status === 'pending')  return { statusCode: 403, headers, body: JSON.stringify({ error: 'Your account is awaiting approval.' }) };
    ...
    await logEvent(email, 'login', email);
    return { statusCode: 200, headers, body: JSON.stringify({ ok: true, user: { email: user.email, name: user.name, role: user.role, initials: user.initials, title: user.title } }) };
  }
